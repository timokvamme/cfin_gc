...
class eyeLink:
    ...
    def calibrate(self):
        ...
        if self.mode == 'Real':
            ...
            self.win.flip()
            drawText(self.win, 'Press "Space" to start!', textSize=self.textSize)
    ...
    def isGazeOutside(self, maxDist=1.5, position=(0,0)):
        ...
        avgXY = pixelsToAngleWH((int(avgXY[0]), int(avgXY[1])), self.screenDist,
                                (self.screenWidth, self.screenHeight),
                                (self.displayResolution[0], self.displayResolution[1]))

        distance = distBetweenPoints(avgXY, position)


        print("avgXY %s" % str(avgXY))
        print("fixDot.pos %s" % str(position))
    ...
    def waitForFixation(self, fixDot, maxDist=1.5, maxWait=4, nRings=3, fixTime=200, etFixProtocolPath="Z:\\MINDLAB2018_MEG-TrainingNCC\ET_Fix_Protocol\etFixProtocol.txt"):
        """
        ...
        while (time.time() - trStart) < maxWait:
            if self.mode != 'Dummy':
                try:
                    ...
                    fixation = pixelsToAngleWH((int(fixation[0]), int(fixation[1])), self.screenDist,
                                               (self.screenWidth, self.screenHeight),
                                               (self.displayResolution[0], self.displayResolution[1]))
                    # print(fixation)
                    # print(fixDot.pos)
                    ...
                    print("could not get eye position, most likely None Type")
            else:
                avgXY = self.getCurSamp()
                avgXY = pixelsToAngleWH((int(avgXY[0]), int(avgXY[1])), self.screenDist,
                                           (self.screenWidth, self.screenHeight),
        ...
        if not correctFixation:
            ...
            try:
                incorrectFixationProtocol =  open(etFixProtocolPath,"r")
                incorrectFixationProtocol = incorrectFixationProtocol.readlines()[0]
                print("etFixProtocolPath opened and incorrectFixationProtocol set to %s" % incorrectFixationProtocol)
            except:
                print("could not open etFixProtocolPath, incorrectFixationProtocol set to user")
                incorrectFixationProtocol = "user"


            if incorrectFixationProtocol == "user":
        ...
        #self.stopRecording()
        return correctFixation, problemWithFixation, Recalibrate, StopGC
